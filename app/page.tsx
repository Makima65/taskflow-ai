...
function FancyEditButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      ...
      className="group/btn relative flex h-8 w-[85px] overflow-hidden items-center justify-start rounded-lg border-none bg-[rgb(168,38,255)] px-4 text-xs font-medium text-white shadow-[4px_4px_0px_rgb(140,32,212)] transition-all duration-300 active:translate-x-[3px] active:translate-y-[3px] active:shadow-[2px_2px_0px_rgb(140,32,212)]"
    >
      <span className="transition-all duration-300 group-hover/btn:-translate-y-8 group-hover/btn:opacity-0">Edit</span>
      <svg
        className="absolute right-4 w-[11px] fill-white transition-all duration-300 group-hover/btn:right-[calc(50%-5.5px)]"
        viewBox="0 0 512 512"
      >
...
function FancyDeleteButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      ...
      className="group/btn relative flex h-8 w-[85px] overflow-hidden items-center justify-start rounded-lg border-none bg-red-500 px-3 text-xs font-medium text-white shadow-[4px_4px_0px_rgb(185,28,28)] transition-all duration-300 active:translate-x-[3px] active:translate-y-[3px] active:shadow-[2px_2px_0px_rgb(185,28,28)] dark:bg-red-600 dark:shadow-[4px_4px_0px_rgb(153,27,27)]"
    >
      <span className="transition-all duration-300 group-hover/btn:-translate-y-8 group-hover/btn:opacity-0">Delete</span>
      <svg
        className="absolute right-3 w-[12px] fill-white transition-all duration-300 group-hover/btn:right-[calc(50%-6px)]"
        viewBox="0 0 448 512"
      >
...
function DraggableTaskCard({ task, onDelete, onEdit }: { task: Task; onDelete: (id: string) => void; onEdit: (id: string, newTitle: string) => void }) {
  ...
  // Renamed the parent group to group/card to isolate hover states
  return (
    <div ref={setNodeRef} style={style} {...listeners} {...attributes} className={`z-10 relative group/card ${!isEditing ? "cursor-grab active:cursor-grabbing" : ""}`}>
      <Card className="relative border-l-4 border-l-purple-600 dark:border-l-purple-500 overflow-hidden">
        
        {/* Increased padding-right (pr-[190px]) so text stops before hitting buttons */}
        <CardHeader className="p-4 pr-[190px]">
          ...
          {isEditing ? (
            ...
            <CardTitle className="text-md font-medium leading-snug break-all">{task.title}</CardTitle>
          )}
        ...
        {/* Buttons now only appear when hovering group/card */}
        {!isEditing && (
          <div className="absolute top-4 right-4 flex gap-2 opacity-0 transition-opacity group-hover/card:opacity-100">
            <FancyEditButton onClick={() => setIsEditing(true)} />
            <FancyDeleteButton onClick={() => onDelete(task.id)} />
          </div>
        )}
      </Card>
    </div>
