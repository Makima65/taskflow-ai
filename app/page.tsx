...
// ========================================================================
// 1. FLOATING INPUT (Fixed & Purple Themed)
// ========================================================================
function FloatingInput({ label, type = "text", value, onChange }: { label: string, type?: string, value: string, onChange: (e: any) => void }) {
  // We use standard React logic here. If value.length > 0, it ALWAYS stays floating!
...
export default function Home() {
  ...
  const handleForgotPassword = () => {
    alert("We will set up the Supabase password reset flow later! For now, try to remember it.");
  };

  // ========================================================================
  // 4. THE NEW CUSTOM LOGIN SCREEN (Your UI Design)
  // ========================================================================
  if (!session) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-zinc-50 p-4 dark:bg-zinc-950">
        
        {/* Container translated exactly from your CSS */}
        <div className="w-[350px] bg-white dark:bg-zinc-900 shadow-[0px_5px_15px_rgba(0,0,0,0.35)] dark:shadow-[0px_5px_15px_rgba(0,0,0,0.8)] rounded-[10px] box-border p-[20px_30px]">
          ...
          <form onSubmit={handleAuth} className="w-full flex flex-col gap-[18px] mb-[15px]">
            ...
            {authError && <p className="text-xs font-semibold text-red-500 text-center">{authError}</p>}
    ...
    <main className="min-h-screen bg-zinc-50 p-8 dark:bg-zinc-950">
      <div className="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button variant="ghost" className="rounded-[20px] hover:text-purple-600 dark:hover:text-purple-400" onClick={() => supabase.auth.signOut()}>
            Sign Out
          </Button>
        </div>
      </div>
